...
По завершении разработки фичи должны быть созданы:
...
2. **Запустите разработку через соответствующих агентов:**
   - Research Agent → анализ проекта
   - Developer Agent → реализация
   - Test Engineer Agent → тесты
   - Code Reviewer Agent → ревью
   - Feature Verifier Agent → верификация

3. **Создайте артефакты в папке `feature-artifacts/`:**
   - PROJECT_ANALYSIS.md
   - FEATURE_DECOMPOSITION.md
   - TDD_PLAN.md
